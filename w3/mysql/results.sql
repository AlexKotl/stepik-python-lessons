...
NATURAL JOIN sale
group by name having count(distinct store_id) = 1

-- 8. Получить названия всех складов, с которых продавался только один продукт
select name as products from sale
natural join store
group by name having count(distinct product_id) = 1
...
group by date
order by sum(quantity) desc, date limit 1
